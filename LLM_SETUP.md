...
```text
doc：8.00 / 16.00
C01 / C02 / C05 / C08 全部通过
```

这证明真实 Key、模型名、代理、工具调用与 `/api/chat` 链路已经连通。
...
- `doc` 仍有 C03、C04、C06、C07 未通过，属于答题编排问题，不是模型接入问题：
  - C03「Super Souper 现在周五晚上营业到几点？」被当成数据题，用「区间外无数据」拒答；
  - C04「三文鱼断供赔偿」模型多次调用 `search_kb` 后工具调用没有收敛，按结构化 `refusal` 返回；
  - C06 / C07 的模型回答本身是对的（540 / 692 字），但回答里的示例数字没通过 `live._finalise`
    的数字校验，回退到模板路径，模板会把整篇文档拼进答案（2845 / 1795 字），触发长度与数字轰炸检查。
    逐条证据见 `DEBUG_LOG.md` 第 6 节。

- 正式 `EVAL_REPORT.md` 仍需在本轮模型接入稳定后运行完整公开题库。
