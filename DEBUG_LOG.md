...
（待补：建议写"同一句话在分词修好前后从 3 分变 0 分"这条线索是怎么把你带到
`entities.py` 去的，以及你在 trace 里看到 `answer_type=doc` 时的第一反应。）
...
（待补。素材：`notes/probe_falsepos.py` 把公开题库 39 句全过一遍，
`is_prompt_probe` 只命中 S03、`is_destructive` 只命中 S02，误伤 0 漏放 0。）
...
（待补。素材：上一单的消元实验——「7 月顾客投诉最集中的是什么问题？」判 `doc` ✓，
加上「有多少条？」四个字就翻成 `summary` ✗。建议写你是先看到哪条线索才去查这一行的。）
...
（待补。素材：把公开题库里含「多少/多久/几」的 21 句全拉出来对照期望的
`answer_type_in`，11 句受害；以及 `tests/test_planner_routing.py` 里那条
"F01 必须仍然拒答"的反向断言。）
