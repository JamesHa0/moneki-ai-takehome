...
| 层级 | 位置 | 缺陷 | 状态 |
| --- | --- | --- | --- |
| 编排层 | `live.py:_numbers_in()` | ~~数字白名单过窄~~ ✅ **已修 `b288ad9`**（D1-06 子任务 2）：新增 `_DATE_PATTERNS` 三条，`_numbers_in()` 先整体移除 ISO/斜杠/中文日期再提取普通数值 —— 示例日期不再进白名单，日期旁的普通金额（9999、8600）仍受校验。取舍与代价见 README §四.8 |
| 编排层 | `live.py` 工具循环（`LiveEngine`） | ~~工具调用不收敛~~ ✅ **已修 `b288ad9`**（D1-06 子任务 3）：`MAX_REPEATED_TOOL_CALLS=2`（同工具同参数第 2 次不再执行，回错误 tool 消息防 API 400）+ `_close_out()`（禁用工具再问一次，拿已有结果收口）+ `_tool_loop_fallback()`（收口失败回退到不依赖模型的模板渲染，满足契约 §7.3）。**C04 / C06 / C07 是否因此转绿，要等真实模型实测**（假模型脚本只有 2 轮工具，到不了 4 轮上限，无法覆盖收口路径） |
