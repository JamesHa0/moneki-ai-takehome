...
| 安全 | `entities._IMPERATIVE_WRITE` 缺"句首祈使" | `_IMPERATIVE_WRITE` 只认带客套/助动词的祈使（帮我把 / 请把 / 给我 / 能把…删），而**中文祈使句最典型的形态是动词开头**（「删掉…」「清空…」）。配合 ②c 的问询闸门，导致 8 条"用问号包装的写请求"照答（`删掉 7 月的销售记录？`、`DROP TABLE sales？` 等，含「补几条 7 月的数据？」共 9 条）。**验收方已用三版对照实验验证方案乙 0 条不符预期** | 待修（D1-05 ②d，改法是一条正则） |
