...
| 7  | `_doc_block()` 挑句的排序方向反了（**已定位，未修复**） | 编排层 | — |
...
## 7. `_doc_block()` 挑句的排序方向反了（已定位，未修复）
...
### 修复方向（未修，工作令见 `notes/D1-tasks.md` D1-07）

`candidates.sort(..., reverse=True)`。**顺带要确认**：`best` / `best_score` 因此变成最大值，
那道 `0.6 * best` 闸门会开始生效 —— 这是预期行为（C03 在模拟里只剩 1 条引用，正好符合它的期望），
但要跑一遍全套确认没有题因为"引用被闸门砍掉"而变红。

### 附带发现（顺手清理项）

`answerer._context()` 在第 6 条的改动之后已经是**死代码**（全仓只有定义、零调用），
而它的 docstring 还写着"把命中的那篇文档原样拼进来，答案就在里面，别漏了"——
正是那条错误理由的原文。建议删掉，或至少把 docstring 改成"保留以便回溯，不要再调用"。
...
| `C02 fact_all` / `C05 fact_any` | 编排层（删整篇拼接） | ⑫ | `_doc_block()` 排序方向 | 待修（见第 7 节） |
| `T02#1 fact_any` / `V03 fact_all` | 编排层（删整篇拼接） | ⑫ | `_doc_block()` 排序方向 | 待修（见第 7 节） |
...
- ⑫ **挑句的排序方向反了**：`_doc_block()` 升序取到最低分的句子，连带 `0.6 * best` 闸门失效。
  这四条检查原本靠「整篇文档被拼进答案」蒙对，删掉拼接后诚实变红 —— 完整证据与模拟见第 7 节。
...
| 编排层 | `answerer.py` `_doc_block()` | **挑句排序方向反了**：`candidates.sort(key=(score, effective_from))` 是升序 → 引用的永远是得分最低的句子；连带 `best = candidates[0][score]` 取到最小值，`0.6 * best` 那道闸门永不触发。实测 8 道 doc 题引用正确数 2 → 7（模拟 `reverse=True`，见第 7 节） | **待修（最高优先）** |
| 编排层 | `answerer.py:352` `_answer_doc()` | **回退路径把整篇文档拼进对外答案**：`self._context(result) + body` 里 `_context()` 拼的是 `hits[0]` 整篇文档。任何一次回退都必然超 `MAX_ANSWER_CHARS = 1200`（实测 C06 2845 字 / C07 1795 字）。改法：`_answer_doc` 只返回 `body`，或对拼接结果设上限 | 待修（**最高优先**，解锁 `doc`） |
| 编排层 | `live.py:_allowed_numbers()` | **数字白名单过窄**：只收集工具结果 + 引用文档 + 问句里的数字，模型用来说明规则的举例日期（「8 月 30 日的订单」）被误判为编造 → 触发回退。日期被 `_DATE_LIKE` 拆成裸数字后按普通数值比较，也是它过窄的一部分 | 待修 |
| 编排层 | `live.py` 工具循环 | **工具调用不收敛**：同一工具被反复调用时没有上限，直到预算耗尽（C04 实测 5 轮） | 待修 |
