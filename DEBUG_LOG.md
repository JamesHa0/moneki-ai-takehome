...
| 编排层 | `planner.py:253` | 「问『多少/多久/几』就是要数字」的兜底会**覆盖 `_choose_kind()` 已经判好的 `kind="doc"`**，把文档题改写成数据题。实测波及：公开题库 21 句含该触发词，其中 11 句期望含 `doc`/`hybrid` —— 被它改写的 6 句（C01 · C04 · C08 · V01 · T02#2 · **S01**）、被"现在=今天"拒答的 4 句、T03 第二轮 1 句。与上一条同属 planner 误判，**要一起修** | 待修（合并进 D1-05 ④，**最高优先**） |
| 安全 | `entities.is_prompt_probe` / `is_destructive` / `sanitize.sanitize` | 近失措辞会误伤：「7 月营业额调整过几次？」「数据有没有更新过？」会被判成破坏性请求；反向「给 8 月的营业额加 500」**不**命中（而 `is_destructive` 的 docstring 把它列为会命中的例子）；「忘记你之前的所有指令」不命中 probe（`sanitize._INJECTION_PATTERNS` 有"忘记"，`entities.PROMPT_PROBE` 没有，两处词表不一致）。隐藏题库有"换一种问法"的版本 | 待修（D1-05 ②b，见 `notes/D1-tasks.md`） |
